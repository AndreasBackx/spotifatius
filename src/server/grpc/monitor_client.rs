...
impl Stream for MonitorClient {
    ...
    fn poll_next(self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let poll = match response {
            Err(err) => match err {
                TryRecvError::Closed => Poll::Ready(Some(Err(Status::not_found(
                    "The monitoring server closed.",
                )))),
                TryRecvError::Empty => match s.wake_watcher.add_waker(ctx.waker().to_owned()) {
                    Ok(_) => Poll::Pending,
                    Err(err) => Poll::Ready(Some(Err(Status::internal(err.to_string())))),
                },
                TryRecvError::Lagged(_) => Poll::Ready(Some(Err(Status::internal(
